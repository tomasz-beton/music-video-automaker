...
def get_beat_times(filename=None, y=None, sr=None):
    ...
    if y is not None and sr is not None:
        ...
        raise ValueError("Something wrong with the arguments")
...
def get_audio_features(audio: MediaFile):
    """Returns AudioFeatures object"""
    # TODO cache

    beat_times, tempo = get_beat_times(filename=audio.path)
    length = librosa.get_duration(filename=audio.path)
    first_beat = beat_times[0]

    return AudioFeatures(audio_id=audio.id, tempo=tempo, beat_times=beat_times, first_beat=first_beat, length=length)
